...
import random
import math
from uav_comms_interface import ROS_interface
#from gazebo_config_subscriber import GazeboRobotControl
...
class FlockingSpots:
    def __init__(self, width=800, height=600, num_spots=30):
        ...
        self.uav_bank_angle = 0  # UAV bank angle in degrees
        self.uav_pitch_angle = 0  # UAV pitch angle in degrees
        self.uav_elevation = 0  # UAV elevation angle in degrees
    ...
    def draw_spots(self):
        ...
        # Generate grey noise
        noise = np.random.randint(150, 200, (self.height, self.width, 3), dtype=np.uint8)
    
        # Blend the noise with the blank image (adjust alpha for intensity)
        alpha = 0.5  # Adjust transparency of the noise (0 = no noise, 1 = full noise)
        ...
        for spot in self.spots:
            if self.target_pos:
                ...
                color = ((spot['brightness']+intensity)%255, (spot['brightness']+intensity)%255,(spot['brightness']+intensity)%255)
            else:
        ...
        if self.cluster_rect:
            ...
            if angle is not None:
                self.uav_elevation = angle
                self.uav_distance = distance
                self.uav_bearing = rel_angle
                cluster_center = ((x1 + x2)/2, (y1 + y2)/2)
                ...
                cv2.putText(img, f"UAV Bank Angle: {self.uav_bank_angle:.1f}deg", 
                           (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                cv2.putText(img, f"UAV Pitch Angle: {self.uav_pitch_angle:.1f}deg",
                            (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)               
        
        cv2.putText(img, "WASD:Move|QE:Rot|+/-:Alt|Click:MoveFlock|R:Reset|p/i:bank|l/o:pitch  ", 
...
def main():
    ...
    while True:
        ...
        ros_interface.update_metrics(
            ...
            bank_angle=flock.uav_bank_angle,
            pitch_angle=flock.uav_pitch_angle
        )
        ros_interface.auto_point_gimbal()
        ...
        if key == 27:  # ESC
            ...
            flock.uav_heading = (flock.uav_heading + 5) % 360
        elif key == ord('p'):  # Rotate clockwise
            flock.uav_bank_angle = max(-45,min(45, flock.uav_bank_angle + 1))
        elif key == ord('i'):  # Rotate counter-clockwise
            flock.uav_bank_angle = max(-45,min(45, flock.uav_bank_angle - 1))
        elif key == ord('o'):  # Rotate clockwise
