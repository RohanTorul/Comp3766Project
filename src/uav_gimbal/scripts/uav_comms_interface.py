...
class ROS_interface:
    ...
    def zowweee(self):
        ...
        bank = math.radians(self.bank_angle)
        heading = math.radians(self.heading)
        elevation = -math.radians(self.elevation)  # Keep original sign
        bearing = math.radians(self.bearing)

        # Compute target direction in UAV-CENTRIC world frame (Z-down)
        # Note: In this frame, positive elevation means target is above UAV
        target_dir = np.array([
            math.cos(elevation) * math.cos(bearing),
            math.cos(elevation) * math.sin(bearing),
            math.sin(elevation)  # Negative because positive elevation -> target above UAV
        ])

        # Compute UAV rotation matrix (world to body)
        R = (self.rotation_matrix_z(heading) @ 
            self.rotation_matrix_y(pitch) @ 
            self.rotation_matrix_x(bank))

        # Transform target to body frame
        target_body = R.T @ target_dir  # Equivalent to inv(R) @ target_dir
        ...
        target_theta1 = -math.atan2(target_body[2], xy_mag)  # Note negative sign
